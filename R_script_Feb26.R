...
# Plot Detection Days
ggplot(plot_data, aes(x = Period_Label, y = Detection_Days, fill = Species)) +
    geom_col(position = "dodge", width = 0.7) +
    scale_fill_brewer(palette = "Set2") +
    labs(
      title = "Detection Days by Survey Period",
      x = "Survey Period",
      y = "Number of Detection Days",
      fill = "Species"
    ) +
...
ggplot(plot_data, aes(x = Period_Label, y = Detection_Prob, fill = Species)) +
    geom_col(position = "dodge", width = 0.7) +
    geom_errorbar(
...
# Plot Detection Probability by Diet Period
plot_data <- results_df %>%
    filter(!is.na(p_Day)) %>%
    pivot_longer(
...
# Create timeline scatter plot for first 20 species each period
First10_by_period %>%
    mutate(
